...
const pTimer = require('./pausableTimer');
            ...
            (
                ...
                if (state.duration > 0) {
                    ...
                    if (self.timeToPlay > 0) {
                        if (self.config.get('enable_debug_logging'))
                            self.logger.info('[Last.fm] Continuing scrobble, starting new timer for the remainder of ' + self.timeToPlay + ' milliseconds [' + state.artist + ' - ' + state.title + '].');
                        ...
                        if (scrobbleThresholdInMilliseconds > 0) {
                            if (self.config.get('enable_debug_logging'))
                                self.logger.info('[Last.fm] starting new timer for ' + scrobbleThresholdInMilliseconds + ' milliseconds [' + state.artist + ' - ' + state.title + '].');
                    ...
                    if (self.config.get('enable_debug_logging'))
                        self.logger.info('[Last.fm] starting new timer for ' + scrobbleThresholdInMilliseconds + ' milliseconds [webradio: ' + state.title + '].');
                    ...
                    callback: function (result) {
                        if (result.success) {
                            ...
                            if (result.correction.artist.name !== undefined && result.correction.artist.name !== '' && self.scrobbleData.artist !== result.correction.artist.name) {
                                self.logger.info('[Last.fm] corrected artist from: ' + self.scrobbleData.artist + ' to: ' + result.correction.artist.name);
                            ...
                            if (result.correction.name !== undefined && result.correction.name !== '' && self.scrobbleData.title !== result.correction.name) {
                                self.logger.info('[Last.fm] corrected track title from: ' + self.scrobbleData.title + ' to: ' + result.correction.name);
                ...
                lfm.scrobbleNowPlayingTrack({
                    ...
                    callback: function (result) {
                        if (!result.success)
                            ...
                            if (self.config.get('enable_debug_logging'))
                                self.logger.info('[Last.fm] updated "now playing" | artist: ' + self.scrobbleData.artist + ' | title: ' + self.scrobbleData.title);
                        }
                    }
                });
            } else {
                ...
                lfm.scrobbleTrack({
                    ...
                    callback: function (result) {
                        ...
                        if (self.config.get('pushToastOnScrobble'))
                            self.commandRouter.pushToastMessage('success', 'Scrobble successful', 'Scrobbled: ' + self.scrobbleData.artist + ' - ' + self.scrobbleData.title + ' (' + self.scrobbleData.album + ').');
                        if (self.config.get('enable_debug_logging'))
                            self.logger.info('[Last.fm] Scrobble successful for: ' + self.scrobbleData.artist + ' - ' + self.scrobbleData.title + ' (' + self.scrobbleData.album + ').');
                    }
                });
            } else {
                self.logger.info("[Last.fm] Error: " + result.error);
...
ControllerLastFM.prototype.generateDependencylist = function () {
    ...
    fs.readdir(__dirname + "/node_modules", function (err, dirs) {
        ...
        dirs.forEach(function (dir) {
            if (dir.indexOf(".") !== 0) {
                const packageJsonFile = __dirname + "/node_modules/" + dir + "/package.json";
